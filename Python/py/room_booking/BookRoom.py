...
import datetime as dt              # Import datetime for handling dates and times
import tkinter as tk               # Import tkinter for GUI components
from tkinter import ttk, messagebox # Import ttk for modern widgets and messagebox for popups
import os                          # Import os for file operations
import csv                         # Import csv for reading/writing booking records
from .rooms_data import ROOMS       # Import predefined rooms data (dictionary)

BOOKINGS_FILE = os.path.join("data", "bookings.csv")  # Path to CSV file storing bookings
USERS_FILE = os.path.join("data", "users.txt")        # Path to CSV file storing student accounts


# ---------------- Student Data ----------------
def load_students():
    """Load student data {id: USERNAME} (convert username to UPPERCASE)"""
    if not os.path.exists(USERS_FILE):   # If file not found, return empty dictionary
        return {}
    students = {}
    with open(USERS_FILE, "r", encoding="utf-8") as f:
        for line in f:
            ...
            if len(parts) == 3:          # Format: student_id, username, password
                sid = parts[0].strip()
                username = parts[1].strip().upper()   # Convert username to uppercase
                students[sid] = username              # Store mapping {id: USERNAME}
    return students


STUDENTS = load_students()   # Load student dictionary once globally


# ---------------- Helpers ----------------
def get_next_5_days():
    """Return a list of 5 consecutive dates (YYYY-MM-DD) starting from today"""
    today = dt.date.today()
    return [(today + dt.timedelta(days=i)).strftime("%Y-%m-%d") for i in range(5)]


def generate_times(start_hour=8, end_hour=21, step=30):
    """Generate a list of times (e.g. 08:00 AM → 09:00 AM …)"""
    times, current = [], dt.datetime(2000, 1, 1, start_hour, 0)  # Start from dummy date
    end = dt.datetime(2000, 1, 1, end_hour, 0)
    while current <= end:
        times.append(current.strftime("%I:%M %p").lstrip("0"))  # Format as 12-hr with AM/PM
        current += dt.timedelta(minutes=step)  # Step forward by 30 minutes
    return times


DATES, TIMES = get_next_5_days(), generate_times()  # Preload date and time options



# ---------------- UI ----------------
def build_page(parent, selected_venue=None, current_user=None, back_callback=None):
    """Build the booking UI page inside parent frame"""
    for w in parent.winfo_children():   # Clear existing widgets
        w.destroy()

    # Scrollable area
    canvas = tk.Canvas(parent, bg="#ecf0f1", highlightthickness=0)  # Canvas for scrolling
    ...
    canvas.create_window((0, 0), window=scroll_frame, anchor="nw") # Place scroll_frame in canvas
    canvas.configure(yscrollcommand=scrollbar.set)

    canvas.pack(side="left", fill="both", expand=True) # Fill horizontally
    scrollbar.pack(side="right", fill="y")

    frm = scroll_frame   # Alias for readability
