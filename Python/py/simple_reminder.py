...
class ReminderApp:
    ...
    def check_reminders(self):
        ...
        # Format the current time as a string (same format as stored in reminders, e.g. "2025-09-18 09:30 AM")

        events = load_reminders(self.current_user)  
        # Load all reminders for the current user from CSV

        for e in events:  
            # Loop through each reminder event
            event_dt = datetime.strptime(e["datetime"], "%Y-%m-%d %I:%M %p")  
            # Convert the reminder’s datetime string into a datetime object for comparison

            # ✅ Normal on-time reminder
            if e["datetime"] == now_str and e["status"] == "Pending":
                update_reminder_status(self.current_user, e["id"], "Ringing")  
                # Update reminder status from "Pending" → "Ringing"

                self.root.after(0, lambda t=e["task"], rid=e["id"], r=e.get("repeat"), dt=e["datetime"]:
                                self.alert(t, rid, r, dt))  
                # Immediately trigger the alert popup and sound

            # ✅ Catch-up logic: Missed reminder (event time < now, still Pending)
                ...
                self.root.after(500, lambda t=e["task"], rid=e["id"], r=e.get("repeat"), dt=e["datetime"]:
                                self.alert(t, rid, r, dt))  
                # Trigger the alert with a slight delay (0.5 seconds)

        if self.root.winfo_exists():  
            # If the main window still exists (not closed by user)
            self.root.after(1000, self.check_reminders)  
            # Schedule this function to run again in 1 second (continuous checking loop)
